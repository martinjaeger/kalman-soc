...
void SoCKalman::init(bool isBattery12V, bool isBatteryLithium, uint32_t batteryEff,
...
void SoCKalman::f(bool isBatteryInFloat, int32_t batteryMilliWatts,
                  uint32_t samplePeriodMilliSec, uint32_t batteryCapacity)
{
    uint32_t milliSecToHours = 3600000;

    int32_t energyChange = ((batteryMilliWatts / 1000) * _batteryEff *
        (samplePeriodMilliSec / milliSecToHours));   // result: Wh

    // Coulomb counting with Wh instead of Ah... does it make sense?
    uint32_t newSoC = (_x[0] * batteryCapacity + energyChange) / batteryCapacity;
...
void SoCKalman::h(int32_t batteryMilliAmps)
{
    ...
    const uint32_t dummyLeadAcidVoltage[101] = {
        11640, 11653, 11666, 11679, 11692, 11706, 11719, 11732, 11745, 11758,
        11772, 11785, 11798, 11811, 11824, 11838, 11851, 11864, 11877, 11890,
        11904, 11917, 11930, 11943, 11956, 11970, 11983, 11996, 12009, 12022,
        12036, 12049, 12062, 12075, 12088, 12102, 12115, 12128, 12141, 12154,
        12168, 12181, 12194, 12207, 12220, 12234, 12247, 12260, 12273, 12286,
        12300, 12313, 12326, 12339, 12352, 12366, 12379, 12392, 12405, 12418,
        12432, 12445, 12458, 12471, 12484, 12498, 12511, 12524, 12537, 12550,
        12564, 12577, 12590, 12603, 12616, 12630, 12643, 12656, 12669, 12682,
        12696, 12709, 12722, 12735, 12748, 12762, 12775, 12788, 12801, 12814,
        12828, 12841, 12854, 12867, 12880, 12894, 12907, 12920, 12933, 12946,
        12960
    };
    const uint32_t dummyLithiumVoltage[101] = {
        5000, 6266, 7434, 8085, 8531, 8867, 9134, 9355, 9543, 9705,
        9847, 9974, 10088, 10191, 10285, 10372, 10451, 10525, 10595, 10659,
        10720, 10777, 10831, 10882, 10931, 10977, 11021, 11063, 11104, 11142,
        11180, 11216, 11251, 11284, 11317, 11349, 11379, 11409, 11438, 11467,
        11495, 11522, 11548, 11574, 11600, 11625, 11650, 11675, 11699, 11723,
        11746, 11769, 11793, 11815, 11838, 11861, 11883, 11906, 11928, 11950,
        11972, 11994, 12017, 12039, 12061, 12083, 12105, 12127, 12150, 12172,
        12195, 12217, 12240, 12263, 12286, 12309, 12333, 12356, 12380, 12404,
        12428, 12452, 12477, 12501, 12526, 12552, 12577, 12603, 12629, 12655,
        12682, 12708, 12735, 12763, 12790, 12818, 12846, 12875, 12903, 12931,
        12960
    };
    const uint32_t dummyOcvSoc[101] = {
    ...
    const uint32_t *dummyVoltage = _isBatteryLithium ? dummyLithiumVoltage : dummyLeadAcidVoltage;

    for (i = 0; i < 101; i++) {
        if (dummyOcvSoc[i] > (uint32_t)_x[0]) {
            // predict voltage for current SoC and measured current using simple equivalent-circuit
            // model with one series resistor _x[1] (_x[2] not clear)
            _h = (dummyVoltage[i] + dummyVoltage[i - 1]) * multiplier / 2 +     // avg voltage
                (batteryMilliAmps / 1000 * _x[1] / 100) + _x[2] / 100;          // + R*I + ?

            _H[0] = (dummyVoltage[i] - dummyVoltage[i - 1]) * multiplier * 100 /
...
void SoCKalman::sample(bool isBatteryInFloat, int32_t batteryMilliAmps,
                       uint32_t batteryVoltage, int32_t batteryMilliWatts,
                       uint32_t samplePeriodMilliSec, uint32_t batteryCapacity)
{
    float temp0[_n * _n];
    float temp1[_n * _n];
...
uint32_t SoCKalman::calculateInitialSoC(uint32_t batteryVoltage)
{
    ...
    const uint16_t battSoCVoltages[VOLTAGES_SIZE] = {
        12720, 12600, 12480, 12360, 12240, 12120, 12000, 11880, 11760, 11640
    };
...
void SoCKalman::matMult(float* a, float* b, float* result, uint8_t arows, uint8_t acols,
                        uint8_t bcols)
{
    int i, j, k;
...
uint8_t SoCKalman::inverse(float* a, float* result)
{
    ...
    for (i = 0; i < 3; i++) {
        determinant = determinant + (a[i] *
            (a[3 + (i + 1) % 3] * a[6 + (i + 2) % 3] - a[3 + (i + 2) % 3] * a[6 + (i + 1) % 3]));
    }
